...
void RegExFor1c::regexMatch(tVariant* paParams, tVariant* pvarRetValue)
{
	...
	{
		sconv.DiagToV8String(pvarRetValue, iMemoryManager, false, L"Первый параметр - строка, в которой будет производиться поиск совпадений.");
		return;
	}

	if (TV_VT(&paParams[1]) != VTYPE_PWSTR)
	{
		sconv.DiagToV8String(pvarRetValue, iMemoryManager, false, L"Второй параметр - регулярное выражение.");
		return;
	}

	wchar_t* wchTxt = nullptr;
	sconv.convFromShortWchar(&wchTxt, (&paParams[0])->pwstrVal);

	if (!wchTxt)
	{
		std::wstringstream wssLoc;
		wssLoc << L"Ошибка работы convFromShortWchar: вероятно проблемы с выделением памяти под первый параметр.";
		sconv.DiagToV8String(pvarRetValue, iMemoryManager, false, wssLoc.str().c_str());
		return;
	}
	...
	sconv.convFromShortWchar(&wchRegExpr, (&paParams[1])->pwstrVal);
...
void RegExFor1c::regexReplace(tVariant* paParams, tVariant* pvarRetValue)
{
	if (TV_VT(&paParams[0]) != VTYPE_PWSTR)
	{
		sconv.DiagToV8String(pvarRetValue, iMemoryManager, false, L"Первый параметр - строка, в которой будет производиться замена.");
		return;
	}
	...
	{
		sconv.DiagToV8String(pvarRetValue, iMemoryManager, false, L"Третий параметр - паттерн для замены.");
		return;
	}
	...
	sconv.convFromShortWchar(&wchPattern, (&paParams[2])->pwstrVal);
	if (!wchPattern)
	{
		std::wstringstream wssLoc;
