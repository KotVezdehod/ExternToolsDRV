            ...
            if (!subschemaSet.insert(subschema).second) {
                throwRuntimeError(
                        //"Failed to store pointer for new sub-schema");
                            "Не удалось сохранить указатель для новой подчиненной схемы");
            }
#if VALIJSON_USE_EXCEPTIONS
        } catch (...) {
            subschema->~Subschema();
    ...
    Subschema *newSubschema()
    {
        ...
        if (!ptr) {
            throwRuntimeError(
                    ...
                    "Не удалось выделить память под новую разделяемую подчиненную схему");
        }
    ...
    Subschema * mutableSubschema(const Subschema *subschema)
    {
        ...
        if (subschema == sharedEmptySubschema) {
            throwRuntimeError(
                    ...
                    "Не удалось изменить разделяемую пустую подчиненную схему");
        }
        ...
        if (subschemaSet.find(noConst) == subschemaSet.end()) {
            throwRuntimeError(
                    //"Subschema pointer is not owned by this Schema instance");
                     "Указатель на подчиненную схему не принадлежит текущему экземпляру родительской схемы");
        }
